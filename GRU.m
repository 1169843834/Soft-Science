%% run_gru_compare_full_metrics.m
clear; clc; close all;
rng(2026, "twister");
...
outDir = fullfile(cfg.resultsRoot, 'GRU');
...
for fold = 1:cfg.kFolds
    ...
    saveTrainingCurve(info, outDir, fold, 'GRU');
...
modelFilePath = fullfile(outDir, 'trained_gru_model.mat');
...
saveAllOutputs(outDir, 'GRU', cfg.kFolds, foldAccuracy, foldMacroF1, foldRRMSE, foldTrainTime, ...
...
sumX = zeros(numFeatures,1); sumX2 = zeros(numFeatures,1); N = 0;
for i = 1:numel(XCell)
    x = double(XCell{i});
    sumX = sumX + sum(x,2);
...
classes = categories(YTrue); numClasses = numel(classes); N = numel(YTrue);
...
rrmse = rmse / sqrt(mean(Y_onehot.^2, 'all'));
end

function saveTrainingCurve(info, outDir, fold, modelName)
fig = figure('Color','w','Position',[120 120 900 360]);
subplot(1,2,1);
if isfield(info,'TrainingLoss') && ~isempty(info.TrainingLoss), plot(info.TrainingLoss,'LineWidth',1.5); hold on; end
if isfield(info,'ValidationLoss') && ~isempty(info.ValidationLoss) && any(~isnan(info.ValidationLoss))
    plot(info.ValidationLoss,'LineWidth',1.5); legend('Train','Validation','Location','best');
else
    legend('Train','Location','best');
end
xlabel('Iteration'); ylabel('Loss'); title(sprintf('%s - Fold %d Loss', modelName, fold)); grid on;
subplot(1,2,2);
...
if hasTrainAcc, plot(info.TrainingAccuracy,'LineWidth',1.5); hold on; end
if hasValAcc, plot(info.ValidationAccuracy,'LineWidth',1.5); end
xlabel('Iteration'); ylabel('Accuracy (%)'); title(sprintf('%s - Fold %d Accuracy', modelName, fold)); grid on;
...
if isa(net, 'SeriesNetwork') || isa(net, 'DAGNetwork')
    ...
    for i = 1:numel(layers)
        layer = layers(i);
        if isprop(layer, 'Weights') && ~isempty(layer.Weights), numParams = numParams + numel(layer.Weights); end
        if isprop(layer, 'Bias') && ~isempty(layer.Bias), numParams = numParams + numel(layer.Bias); end
        if isprop(layer, 'InputWeights') && ~isempty(layer.InputWeights), numParams = numParams + numel(layer.InputWeights); end
        if isprop(layer, 'RecurrentWeights') && ~isempty(layer.RecurrentWeights), numParams = numParams + numel(layer.RecurrentWeights); end
        if isprop(layer, 'Offset') && ~isempty(layer.Offset), numParams = numParams + numel(layer.Offset); end
        if isprop(layer, 'Scale') && ~isempty(layer.Scale), numParams = numParams + numel(layer.Scale); end
    end
else
    numParams = NaN;
end
...
saveas(fig, fullfile(outDir, 'confusion_matrix.png')); close(fig);
